...
Active v2 rebuild. **Working (confirmed on a real phone):** desktop app, live mirroring, and AnyDesk-style control. **Two connection methods:** **local** (same Wi-Fi — works out of the box) and **remote** (any network / city, via a relay + pairing code — deploy the relay once, see **[REMOTE.md](REMOTE.md)**). UI polish is currently deferred in favour of logic robustness. **Next:** true peer-to-peer WebRTC (take media off the relay), ADB turbo, and the Controller mobile app. Background in **[CONNECTIVITY.md](CONNECTIVITY.md)**; detail in [CHANGELOG.md](CHANGELOG.md) / [MEMORY.md](MEMORY.md); roadmap in [REBUILD-PLAN.md](REBUILD-PLAN.md).
